...
@login_required
def view_cart():
    items = _cart_rows(g.user["user_id"])
...
def add(product_id):
    db = get_db()
    p = db.execute("SELECT product_id, stock FROM products WHERE product_id = ?",
                   (product_id,)).fetchone()
    if p is None:
        abort(404)
    try:
    ...
    db.execute(
        ...
        (g.user["user_id"], product_id, new_qty))
    db.commit()
    log_event(g.user["user_id"], product_id, "added_to_cart")
...
def update(product_id):
    ...
    if qty <= 0:
        ...
        stock = db.execute("SELECT stock FROM products WHERE product_id = ?",
                           (product_id,)).fetchone()
        if stock is None:
            abort(404)
        if qty > stock["stock"]:
...
def checkout():
    ...
    total = sum(r["line_total"] for r in items)
    cur = db.execute(
        "INSERT INTO orders(user_id, total) VALUES (?,?)", (g.user["user_id"], total))
    ...
    for r in items:
        ...
        for _ in range(r["qty"]):
            db.execute(
                ...
                (g.user["user_id"], r["product_id"]))

    db.execute("DELETE FROM cart_items WHERE user_id = ?", (g.user["user_id"],))
    db.commit()
...
def order_confirmation(order_id):
    ...
    order = db.execute(
        ...
        (order_id, g.user["user_id"])).fetchone()
    if order is None:
        abort(404)
    items = db.execute(
